...
export class EventHandlers {
    ...
    setupModalListeners() {
        ...
        document.getElementById('confirm-exit-excavation')?.addEventListener('click', () => {
            this.mapController.exitExcavationMode();
    ...
    setupMeasurementHandlers() {
        ...
        // Measurement toolbar handlers
        document.querySelector('.measurement-button[data-tool="measure-distance"]')?.addEventListener('click', () => {
            this.startMeasurementMode();
        });
        
        document.querySelector('.measurement-button[data-tool="add-note"]')?.addEventListener('click', () => {
            this.startAddingAnnotation();
        });
        
        document.querySelector('.measurement-button[data-tool="clear-measurements"]')?.addEventListener('click', () => {
            this.mapController.clearMeasurements();
            this.uiController.showToast('Measurements cleared', 'info');
        });
        
        document.querySelector('.measurement-button[data-tool="exit-measure"]')?.addEventListener('click', () => {
    ...
    toggleMeasurementMode() {
        if (this.appState.isMeasuringDistance) {
            this.endMeasurementMode();
        } else {
            this.startMeasurementMode();
        }
    ...
    handleMeasurementClick(e) {
        ...
        if (toolType === 'measure-distance') {
            // Collect two points to create a measurement
            if (this.appState.measurePoints.length === 0) {
                this.appState.measurePoints.push(e.latlng);
                this.uiController.showStatusBar('Click second point to complete measurement');
            } else if (this.appState.measurePoints.length === 1) {
                this.appState.measurePoints.push(e.latlng);
                // Add measurement to map
                this.mapController.addMeasurementLine(this.appState.measurePoints);
                // Reset state for next measurement
                this.appState.measurePoints = [];
                // Remove temp preview line if present
                if (this.appState.measureLine) {
                    this.mapController.map.removeLayer(this.appState.measureLine);
                    this.appState.measureLine = null;
                }
                this.uiController.showStatusBar('Measurement added. Click to start another or exit.');
            }
