...
export class MapController {
    ...
    updateHeading(heading) {
        ...
        if (this.userLocationMarker) {
            try {
                ...
                if (markerElement) {
                    // Add .with-heading class to the marker container
                    ...
                    if (!headingElement) {
                        ...
                        // Ensure element is styled properly directly to avoid CSS issues
                        headingElement.style.position = 'absolute';
                        headingElement.style.top = '-24px';
                    ...
                    // Always ensure the heading element is visible
                    headingElement.style.display = 'block';
                    headingElement.style.visibility = 'visible';
                    headingElement.style.opacity = '1';
                    
                    // Apply the rotation transformation
                    // Use translateX to center and rotate to the heading angle
    ...
    addUserLocationMarker(lat, lng) {
        ...
        // Create a very simple marker with a clear heading indicator
        const userLocationIcon = L.divIcon({
            ...
            html: `
                ...
                <div style="position: absolute; width: 10px; height: 10px; border-radius: 50%; background-color: #2196F3; top: 50%; left: 50%; transform: translate(-50%, -50%);"></div>
                <div class="location-heading" style="position: absolute; top: -24px; left: 50%; width: 0; height: 0; border-left: 12px solid transparent; border-right: 12px solid transparent; border-bottom: 24px solid #2196F3; transform-origin: bottom center; transform: translateX(-50%) rotate(0deg); opacity: 1; z-index: 1001;"></div>
            `,
            iconSize: [48, 48],
            iconAnchor: [24, 24]
        });
        ...
        // Set a test heading to ensure visibility - will be overridden by actual heading updates
        setTimeout(() => {
            this.updateHeading(45);
            console.log('Applied test heading: 45 degrees');
        }, 1000);
