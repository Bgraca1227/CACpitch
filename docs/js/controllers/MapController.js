...
export class MapController {
    constructor(appState, dataStore) {
        ...
        // Keep track of the last known location so we can derive a bearing
        // when the device does not supply heading information (or the browser
        // refuses to expose it). This greatly improves the reliability of the
        // on-screen direction indicator which is a long-standing issue.
        this._prevLocationForHeading = null;
    }
                    ...
                    this.appState.highAccuracyWatchId = navigator.geolocation.watchPosition(
                        ...
                        (position) => {
                            ...
                            if (position.coords.heading !== null && 
                                ...
                                // Fallback: derive heading from movement if compass data is unavailable.
                                if (this._prevLocationForHeading) {
                                    const bearing = this._calculateBearing(
                                        this._prevLocationForHeading.lat,
                                        this._prevLocationForHeading.lng,
                                        position.coords.latitude,
                                        position.coords.longitude
                                    );
                                    if (!isNaN(bearing)) {
                                        const filtered = this.locationFilter.addHeading(bearing);
                                        this.updateHeading(filtered);
                                    }
                                }
                            }
        ...
        if (this.userLocationMarker) {
            try {
                ...
                if (markerElement) {
                    ...
                    // Always ensure the heading element is visible and properly styled
                    headingElement.style.display = 'block';
                    headingElement.style.visibility = 'visible';
                    headingElement.style.opacity = '1';
                    headingElement.style.borderBottom = '24px solid #2196F3';
                    
                    // Apply rotation with translateX to ensure proper positioning
        ...
        if (this.appState.activeAlerts) {
            ...
            this.appState.activeAlerts.forEach(alert => {
                ...
                if (!stillCritical) {
                    ...
                    const elapsed = new Date().getTime() - alert.timestamp;
                    if (elapsed > THIRTY_SEC) {
    ...
    updateLocationMarker(position) {
        ...
        this._prevLocationForHeading = { lat, lng };
    ...
    /**
     * Calculate the initial bearing (forward azimuth) between two latitude/longitude pairs
     * using the haversine formula. Returns degrees in the range 0–359, where 0 is north.
     * @private
     */
