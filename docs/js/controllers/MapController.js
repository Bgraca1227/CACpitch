...
export class MapController {
     ...
     * @returns {Array} [lat, lng] of the midpoint
     */
    getMidpoint(p1, p2) {
        return [
            (p1.lat + p2.lat) / 2,
    ...
    calculateDistance(p1, p2) {
        ...
        try {
            ...
            // which uses the Haversine formula for accuracy
    ...
    /**
     * Haversine distance calculation as a fallback
    ...
    /**
     * Format a distance value in feet for display
     * @param {number} distanceInMeters Distance in meters
     * @returns {string} Formatted distance in feet
     */
    formatDistanceImperial(distanceInMeters) {
    ...
    startMeasurement() {
        ...
        // Set cursor to indicate measurement mode
        document.body.style.cursor = 'crosshair';
        
        // Show toast notification
        if (this.uiController) {
            this.uiController.showToast('Measurement mode active', 'info');
        }
    }

    /**
     * Add a permanent measurement line to the map
     * @param {Array} points Array of L.LatLng objects
     */
    addMeasurementLine(points) {
        ...
        try {
            ...
            // Create line with both metric and imperial distances
            ...
            const metricDistance = this.formatDistance(distance);
            const imperialDistance = this.formatDistanceImperial(distance);
            ...
            // Store the measurement for future reference
            this.dataStore.measurements.push({
                id: this.dataStore.generateId('measure'),
    ...
    startHighAccuracyLocationTracking() {
        ...
        if (navigator.geolocation) {
            // Initialize position prediction system for smoother tracking
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                (initialPosition) => {
                    ...
                    // Start watching for device orientation if supported
                    ...
                    this.appState.highAccuracyWatchId = navigator.geolocation.watchPosition(
                        ...
                        (position) => {
                            ...
                            if (this.appState.isExcavationMode && this.appState.locationFollowing) {
                                if (filteredPosition) {
        ...
        if (window.DeviceOrientationEvent) {
            ...
            this.handleDeviceOrientation = (event) => {
                ...
                if (event.alpha !== null) {
                    ...
                    // iOS devices may need webkitCompassHeading
            ...
            // Handle iOS permissions for device orientation
    ...
    updateLocationMarker(position) {
        ...
        // Check for nearby utilities if in excavation mode
        if (this.appState.isExcavationMode) {
            this.checkProximityAlerts();
        }
    }
