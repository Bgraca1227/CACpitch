...
export class MapController {
    constructor(appState, dataStore) {
        ...
        this.locationFilter = {
            positions: [],
            maxPositions: 5,  // Number of positions to keep for filtering
            accuracyThreshold: 20, // Meters - reject positions worse than this
            headings: [],
            maxHeadings: 8,   // Heading samples to keep
    ...
    startMeasurement() {
        ...
        document.getElementById('measurement-toolbar').classList.add('visible');
    ...
    addMeasurementPoint(latlng) {
        ...
        L.marker(latlng, {
            icon: L.divIcon({
                ...
                html: `<span>${this.measurementPoints.length}</span>`,
                iconSize: [20, 20],
                iconAnchor: [10, 10]
            })
        }).addTo(this.measurementLayer);
        
        // If we have at least 2 points, draw a line and show distance
        if (this.measurementPoints.length >= 2) {
            const lastTwoPoints = this.measurementPoints.slice(-2);
            
            // Draw line between the last two points
            const line = L.polyline(lastTwoPoints, {
                color: '#ff6b6b',
                weight: 2,
                opacity: 0.8,
                dashArray: '5, 5'
            ...
            // Calculate distance
            const distance = this.map.distance(lastTwoPoints[0], lastTwoPoints[1]);
            const midpoint = [
                (lastTwoPoints[0].lat + lastTwoPoints[1].lat) / 2,
                (lastTwoPoints[0].lng + lastTwoPoints[1].lng) / 2
            ];
    ...
    formatDistance(distanceInMeters) {
        ...
        // For longer distances, show in kilometers
    ...
    // Calculate planar midpoint between two LatLng points
    getMidpoint(p1, p2) {
    ...
    calculateDistance(p1, p2) {
        const meters = this.map.distance(p1, p2);
        return meters * 3.28084; // feet
    }

    // Add a finished measurement to the dedicated layer and persist meta-data
    addMeasurementLine(points) {
        ...
        const distanceFt = this.calculateDistance(points[0], points[1]);
        const midpoint = this.getMidpoint(points[0], points[1]);
        const labelHtml = `\n            <div class="measure-label">${distanceFt.toFixed(2)} ft</div>\n        `;
        const labelIcon = L.divIcon({ html: labelHtml, className: '', iconSize: [80, 24], iconAnchor: [40, 12] });
        const labelMarker = L.marker(midpoint, { icon: labelIcon, draggable: false }).addTo(this.measurementLayer);
        // Persist optional tracking array on appState / dataStore if desired
        if (this.dataStore && Array.isArray(this.dataStore.measurements)) {
            this.dataStore.measurements.push({
                id: this.dataStore.generateId ? this.dataStore.generateId('measurement') : `measurement-${Date.now()}`,
                points,
                distance: distanceFt,
                line,
                label: labelMarker
