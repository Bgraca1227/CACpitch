...
export class MapController {
    ...
    updateLocationMarker(position) {
        ...
        // Update heading if available from device sensors
        if (position.coords.heading !== null && 
            position.coords.heading !== undefined && 
            !isNaN(position.coords.heading)) {
            const filteredHeading = this.locationFilter.addHeading(position.coords.heading);
            this.updateHeading(filteredHeading);
        } else if (this.appState.lastKnownLocation) {
            // Fallback: calculate bearing based on movement between last and current position
            const { lat: prevLat, lng: prevLng } = this.appState.lastKnownLocation;
            const bearing = this.calculateBearing(prevLat, prevLng, lat, lng);
            if (!isNaN(bearing)) {
                const filteredHeading = this.locationFilter.addHeading(bearing);
                this.updateHeading(filteredHeading);
            }
        }
        
        // Remember the latest location for the next bearing calculation
        this.appState.lastKnownLocation = { lat, lng };
    ...
    /**
     * Calculate compass bearing (in degrees) from point A to point B.
     * This provides a reliable fallback when sensor-based heading data is absent.
     */
