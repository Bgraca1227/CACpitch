...
export class MapController {
    ...
    addUserLocationMarker(lat, lng) {
        ...
        const userLocationIcon = L.divIcon({
            className: 'user-location-marker medium-accuracy', // Remove 'with-heading' class to start without arrow
            html: markerHtml,
            iconSize: [40, 40],
            iconAnchor: [20, 20]
        });
    ...
    setupOrientationTracking() {
        // If already active or already requested, skip
        if (this.isOrientationActive || this.orientationRequested) {
            console.log('Orientation tracking already active/requested');
            return;
        }
                ...
                DeviceOrientationEvent.requestPermission()
                    .then(permissionState => {
                        if (permissionState === 'granted') {
                            ...
                            if (!this.orientationToastShown) {
                                this.uiController.showToast('Compass access granted', 'success');
                                this.orientationToastShown = true;
                            }
                        } else {
                            if (!this.orientationToastShown && this.uiController) {
    ...
    handleOrientation(e) {
        ...
        if (e.webkitCompassHeading !== undefined) {
            ...
            if (e.absolute === true) {
                ...
                console.log('Screen-corrected heading:', heading, 'screen angle:', screenOrientation);
            } else {
                // Fallback to basic alpha conversion
                heading = 360 - e.alpha;
                console.log('Basic alpha heading:', heading);
            }
