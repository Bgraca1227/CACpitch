...
export class MapController {
    ...
    updateHeading(heading) {
        ...
        if (this.userLocationMarker) {
            try {
                ...
                if (markerElement) {
                    // FORCE add .with-heading class to the marker container
                    markerElement.classList.add('with-heading');
                    
                    // Find heading element - if not found, create it completely new
                    let headingElement = markerElement.querySelector('.location-heading');
                    if (!headingElement) {
                        console.log('Creating completely new heading element');
                        headingElement = document.createElement('div');
                        headingElement.className = 'location-heading';
                        markerElement.appendChild(headingElement);
                    }
                    
                    // Direct style override with !important flags to prevent other CSS from interfering
                    headingElement.style.cssText = `
                        ...
                        transform: translateX(-50%) rotate(${heading}deg) !important;
                    ...
                    console.log(`Forcefully updated heading to ${heading} degrees with direct style override`);
                    
                    // Create an extra marker that always shows north for debugging
                    if (!markerElement.querySelector('.debug-north')) {
                        const northMarker = document.createElement('div');
                        northMarker.className = 'debug-north';
                        northMarker.style.cssText = `
                            position: absolute !important;
                            top: -20px !important;
                            left: 50% !important;
                            width: 0 !important;
                        ...
                        `;
                        markerElement.appendChild(northMarker);
                    }
                } else {
                    console.warn('Marker element not found');
                }
            } catch (e) {
                console.error('Error updating heading:', e);
            }
        } else {
    ...
    startWatchingOrientation() {
        ...
        const setupOrientation = () => {
            ...
            // Create a more direct event handler for device orientation
            window.addEventListener('deviceorientation', (event) => {
                // Get heading data directly from the event
                let heading = null;
                
                // Try to get heading data different ways depending on device
                if (event.webkitCompassHeading !== undefined) {
                    ...
                    console.log('Android heading from alpha:', heading);
                }
                
                // If we have a valid heading, update immediately
            ...
            // Also add an absolute orientation event listener for devices that support it
            window.addEventListener('deviceorientationabsolute', (event) => {
                if (event.alpha !== null) {
                    const heading = 360 - event.alpha;
                    console.log('Absolute orientation heading:', heading);
                    this.updateHeading(heading);
                }
            }, {passive: true});
            
            // Set a fallback test heading after a short delay
            setTimeout(() => {
                // Force a test heading to verify the indicator appears
                this.updateHeading(45);
                console.log('Applied test heading: 45 degrees');
                
                // Check again after a few seconds
                setTimeout(() => {
                    console.log('Current heading state:', this.currentHeading);
                    if (this.currentHeading === 45) {
                        console.log('WARNING: Heading has not changed from test value');
                        // Try a different heading to see if it updates
                        this.updateHeading(90);
                    }
                }, 5000);
            }, 2000);
        };
        
        // Try to set up right away first
        setupOrientation();
        
        // Also request permission if needed (iOS 13+)
            ...
            DeviceOrientationEvent.requestPermission()
                .then(response => {
                    if (response === 'granted') {
                        ...
                        console.log('Orientation permission denied, using fallback');
                        // Apply a static test heading since we can't get real data
                        this.updateHeading(0);
                    }
                })
                .catch(err => {
                    console.error('Error requesting orientation:', err);
                    // Apply a static test heading since we can't get real data
                    this.updateHeading(0);
                });
        }
    ...
    updateProximityAlerts(utilitiesInProximity) {
        ...
        // Sort utilities by distance (closest first)
        utilitiesInProximity.sort((a, b) => a.distance - b.distance);
        ...
        // Get the proximity warning box
        const warningBox = document.getElementById('proximity-warning-box');
        
        // Track if we're near a utility for warning box display
        let isNearUtility = false;
        let closestUtility = null;
        
        // Process each utility - limited to 3 alerts at a time to prevent overload
        for (let i = 0; i < Math.min(utilitiesInProximity.length, 3); i++) {
            const item = utilitiesInProximity[i];
            const utility = item.utility;
            const distance = item.distance;
            const layer = item.layer;
            
            // Update utility highlighting
            this.highlightUtilityByProximity(layer, distance);
            ...
            // Get appropriate class based on distance
            let proximityClass = '';
            if (distance < 1) {
                proximityClass = 'proximity-critical';
                this.playProximityAlertSound('critical');
            } else if (distance < 3) {
                proximityClass = 'proximity-danger';
                this.playProximityAlertSound('danger');
            } else if (distance < 5) {
                proximityClass = 'proximity-caution';
            } else if (distance < 15) {
                proximityClass = 'proximity-warning';
            }
            
            alert.classList.add(proximityClass);
            
            // Construct alert content
            alert.innerHTML = `
                <div class="proximity-icon ${utility.type}">
                    <i class="fas ${this.getUtilityIcon(utility.type)}"></i>
                </div>
                <div class="proximity-info">
                    <div class="proximity-title">${this.getUtilityTitle(utility)}</div>
                    <div class="proximity-distance">${distance.toFixed(1)}m away</div>
                </div>
                <div class="proximity-depth">Depth: ${utility.depth}ft</div>
            `;
            
            // Add to container
            alertsContainer.appendChild(alert);
            
            // Update warning box info if this is the closest utility
            if (i === 0) {
                isNearUtility = true;
                closestUtility = {
                    utility: utility,
        ...
        // Handle warning box visibility
        if (isNearUtility && closestUtility) {
            // Clear any existing timeout
            if (this.warningBoxTimeout) {
                clearTimeout(this.warningBoxTimeout);
                this.warningBoxTimeout = null;
            }
            
            // Update warning box content
            if (warningBox) {
                const typeSpan = warningBox.querySelector('.warning-type');
                const distanceSpan = warningBox.querySelector('.warning-distance');
                
                if (typeSpan) typeSpan.textContent = closestUtility.utility.type.charAt(0).toUpperCase() + closestUtility.utility.type.slice(1);
                if (distanceSpan) distanceSpan.textContent = closestUtility.distance.toFixed(1) + 'm';
                
                warningBox.classList.add('visible');
                
                // Track the current utility ID
                this.currentProximityUtilityId = closestUtility.utility.id;
            }
        } else {
            // If we're not near a utility but the warning box is visible
            if (warningBox && warningBox.classList.contains('visible') && !this.warningBoxTimeout) {
                // Start a 30-second timer to hide the warning
                this.warningBoxTimeout = setTimeout(() => {
                    warningBox.classList.remove('visible');
                    this.warningBoxTimeout = null;
                    this.currentProximityUtilityId = null;
                }, 30000); // 30 seconds
            }
        }
    }
    ...
    addUserLocationMarker(lat, lng) {
        ...
        // Create a user location marker with a highly visible heading indicator
        const userLocationIcon = L.divIcon({
            className: 'user-location-marker with-heading',
            html: `
                <div class="location-accuracy-circle" style="position: absolute; width: 100%; height: 100%; border-radius: 50%; background-color: rgba(33, 150, 243, 0.15);"></div>
                <div style="position: absolute; width: 14px; height: 14px; border-radius: 50%; background-color: #2196F3; top: 50%; left: 50%; transform: translate(-50%, -50%); box-shadow: 0 0 5px rgba(0,0,0,0.3);"></div>
                <div class="location-heading" style="position: absolute !important; top: -24px !important; left: 50% !important; width: 0 !important; height: 0 !important; border-left: 14px solid transparent !important; border-right: 14px solid transparent !important; border-bottom: 28px solid red !important; transform-origin: bottom center !important; transform: translateX(-50%) rotate(0deg) !important; z-index: 10000 !important; display: block !important; visibility: visible !important; opacity: 1 !important;"></div>
            `,
            iconSize: [48, 48],
            iconAnchor: [24, 24]
        });
        ...
        // Set an initial heading to make the direction indicator visible
        setTimeout(() => {
            this.updateHeading(45);
            console.log('Applied initial test heading: 45 degrees');
            
            // Apply another test heading after a short delay to ensure it updates
            setTimeout(() => {
                this.updateHeading(90);
                console.log('Applied second test heading: 90 degrees');
            }, 2000);
        }, 1000);
    ...
    /**
     * Find a utility layer by its ID
     * @param {string} utilityId The ID of the utility to find
     * @returns {Object|null} The Leaflet layer for the utility or null if not found
     */
    findUtilityLayerById(utilityId) {
        ...
        for (const type in this.utilityLayers) {
            this.utilityLayers[type].eachLayer(layer => {
                if ((layer.utilityId && layer.utilityId === utilityId) || 
                    (layer.utilityData && layer.utilityData.id === utilityId)) {
                    foundLayer = layer;
                }
            });
    ...
    /**
     * Remove a utility from the map
     * @param {string} utilityId The ID of the utility to remove
     */
    removeUtility(utilityId) {
        if (!utilityId) return;
        
        const layer = this.findUtilityLayerById(utilityId);
        if (layer) {
            // Get the utility type to determine which layer group to remove from
            let utilityType = layer.utilityData?.type || 'unknown';
            
            // Remove from the appropriate layer group
            if (this.utilityLayers[utilityType]) {
                this.utilityLayers[utilityType].removeLayer(layer);
            }
        }
    }
} 