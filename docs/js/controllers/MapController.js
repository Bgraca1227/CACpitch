...
export class MapController {
    ...
    initMap(containerId = 'map') {
        ...
        try {
            ...
            // Start tracking user location immediately
    ...
    /**
     * Start high accuracy location tracking
     * This enables continuous location updates with high accuracy
     */
    startHighAccuracyLocationTracking() {
        ...
        // If we already have a watch ID, clear it first
        if (window.appState.highAccuracyWatchId) {
            navigator.geolocation.clearWatch(window.appState.highAccuracyWatchId);
        }
        
        // Initialize location filter if not exists
        if (!this.locationFilter) {
            this.locationFilter = {
                ...
                // Add a position and get filtered result
                ...
                // Add a heading and get filtered result
                addHeading: function(heading) {
                    this.headings.push(heading);
                    if (this.headings.length > this.maxItems) {
                        this.headings.shift();
                    }
                    return this.getFilteredHeading();
                },
                
                // Get average of recent positions to reduce jitter
                getFilteredPosition: function() {
                    if (this.positions.length === 0) return null;
                    
                    // For single position, just return it
                    if (this.positions.length === 1) return this.positions[0];
                    
                    // For multiple positions, calculate weighted average
                    // with more recent positions having higher weight
                    let sumLat = 0;
                    let sumLng = 0;
                    let sumAcc = 0;
                    let totalWeight = 0;
                    ...
                    for (let i = 0; i < this.positions.length; i++) {
                        // Weight increases for more recent positions
                        const weight = i + 1;
                        totalWeight += weight;
                        
                        sumLat += this.positions[i].coords.latitude * weight;
                        sumLng += this.positions[i].coords.longitude * weight;
                        sumAcc += this.positions[i].coords.accuracy * weight;
                    }
                    ...
                    return {
                        coords: {
                            ...
                            accuracy: sumAcc / totalWeight,
                            heading: this.positions[this.positions.length - 1].coords.heading
                        },
                        timestamp: this.positions[this.positions.length - 1].timestamp
                    };
                },
                
                // Get average of recent headings with special handling for circular values
                getFilteredHeading: function() {
                    if (this.headings.length === 0) return null;
                    if (this.headings.length === 1) return this.headings[0];
                    
                    // Handle the circular nature of heading (0-360)
                    let sinSum = 0;
                    let cosSum = 0;
                    
                    // Calculate the average using trigonometry
                    for (let i = 0; i < this.headings.length; i++) {
                        const angleInRadians = (this.headings[i] * Math.PI) / 180;
                        sinSum += Math.sin(angleInRadians);
                        cosSum += Math.cos(angleInRadians);
                    }
                    
                    // Calculate the average angle
                    let averageAngle = Math.atan2(sinSum, cosSum);
                    
                    // Convert back to degrees
                    let averageHeading = (averageAngle * 180) / Math.PI;
                    
                    // Normalize to 0-360
                    averageHeading = ((averageHeading % 360) + 360) % 360;
        ...
        const watchId = navigator.geolocation.watchPosition(
            ...
            (position) => {
                ...
                window.appState.lastLocationUpdate = Date.now();
                
                // Filter position to reduce jitter
                const filteredPosition = this.locationFilter.addPosition(position);
                
                // Update user location in app state
                ...
                this.updateLocationMarker(filteredPosition);
                ...
                // Check proximity to utilities if in excavation mode
                if (window.appState.isExcavationMode) {
                    this.checkUtilityProximity();
                }
            },
            // Error handler
            (error) => {
                ...
                if (this.uiController) {
                    ...
                    switch (error.code) {
                        ...
                        case error.POSITION_UNAVAILABLE:
                            ...
                            break;
                        case error.TIMEOUT:
                            message = 'Location request timed out. Poor GPS signal?';
                            break;
                    }
        ...
        // Store the watch ID in app state
        window.appState.highAccuracyWatchId = watchId;
        
        // Also start watching device orientation for compass heading
        // THIS IS THE KEY ADDITION: Start watching device orientation to get heading
        this.startWatchingOrientation();
        
        console.log('Started high accuracy location tracking');
    }
     ...
     * This is the missing implementation that was causing heading updates to fail
     */
    startWatchingOrientation() {
        ...
        if (!window.DeviceOrientationEvent) {
            ...
            if (this.uiController) {
                this.uiController.showToast('Compass heading not supported on this device', 'warning');
            }
            return;
        }
        ...
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            console.log('Device requires explicit permission for orientation');
            
            // Create permission request button
            this.createOrientationPermissionButton();
            return;
        }
        ...
        console.log('Started watching device orientation');
    }
    
    /**
     * Create a button to request orientation permission (required on iOS)
     */
    createOrientationPermissionButton() {
        // Check if button already exists
        if (document.getElementById('orientation-permission-btn')) {
            return;
        }
        
        // Create button
        const btn = document.createElement('button');
        btn.id = 'orientation-permission-btn';
        btn.innerHTML = '<i class="fas fa-compass"></i> Enable Compass';
        ...
        btn.addEventListener('click', async () => {
            try {
                ...
                if (this.uiController) {
                    this.uiController.showToast('Compass heading activated', 'success');
                }
            } catch (error) {
                console.error('Error requesting device orientation permission:', error);
                if (this.uiController) {
                    this.uiController.showToast('Could not access compass - please check permissions', 'error');
                }
            }
        });
        ...
        // Show toast explaining the button
        if (this.uiController) {
            this.uiController.showToast('Tap "Enable Compass" to show heading direction', 'info');
        }
    }
    
    /**
     * Set up the actual orientation event listener
     */
    setupOrientationListener() {
        ...
        this._orientationHandler = (event) => {
            // Get device heading from alpha property
            // Alpha is the compass direction the device is facing
            if (event.alpha !== null && !isNaN(event.alpha)) {
                // Store raw alpha
                const rawAlpha = event.alpha;
                
                // Apply webkitCompassHeading if available (iOS)
                let heading = (event.webkitCompassHeading || 360 - rawAlpha) % 360;
                
                // Filter heading to reduce jitter
        ...
        // Add listener
        window.addEventListener('deviceorientation', this._orientationHandler, false);
        
        // Start initial test with a known heading (North)
        setTimeout(() => {
