...
export class MapController {
    ...
    updateHeading(heading) {
        ...
        if (this.userLocationMarker) {
            try {
                ...
                if (markerElement) {
                    // CRITICAL: Ensure the container has the with-heading class
                    ...
                    // Force explicit styling with !important rules and add animation properties
                    // Apply drop shadow and transition for smoother look and animation
                    headingElement.style = `
                        ...
                        border-bottom: 28px solid #2196F3 !important;
    ...
    startWatchingOrientation() {
        ...
        const setupOrientation = () => {
            ...
            this.handleDeviceOrientation = (event) => {
                ...
                // iOS devices use webkitCompassHeading - most reliable source
                ...
                // Android devices typically use alpha with compensation
                    ...
                    if (event.gamma !== null && event.beta !== null) {
                        ...
                        // Enhanced tilt compensation algorithm
                        // Only apply compensation when device is significantly tilted
                        if (Math.abs(gamma) > 10 || Math.abs(beta) > 10) {
                            // Calculate tilt adjustment factor based on severity of tilt
                            const gammaFactor = Math.min(1, Math.abs(gamma) / 90);
                            const betaFactor = Math.min(1, Math.abs(beta) / 90);
                            
                            // Apply proportional adjustment
                            const adjustment = (gamma > 0 ? -20 : 20) * gammaFactor * betaFactor;
                            heading += adjustment;
                ...
                if (heading !== null && !isNaN(heading)) {
                    ...
                    // Use our filter to smooth the heading with higher weight for new readings
                    this.locationFilter.maxHeadings = 5; // Reduce buffer size for faster response
            ...
            // Increase event handler frequency for more responsive updates
            const options = {
                passive: true,
                frequency: 50 // Poll more frequently (50ms) for smoother updates
            };
            
            // Try both absolute and regular orientation events
            if ('ondeviceorientationabsolute' in window) {
                console.log('Using deviceorientationabsolute event');
                window.addEventListener('deviceorientationabsolute', this.handleDeviceOrientation, options);
            } else {
                console.log('Using deviceorientation event');
                window.addEventListener('deviceorientation', this.handleDeviceOrientation, options);
            }
            ...
            setTimeout(() => {
                ...
                // Return to normal heading updates after 2 seconds (reduced from 3)
                setTimeout(() => {
    ...
    updateProximityAlerts(utilitiesInProximity) {
        ...
        criticalUtilities.forEach(item => {
            ...
            if (!existingAlert) {
                ...
                const alert = {
                    ...
                    lastSeenTime: new Date().getTime() // Track when utility was last seen
                };
                ...
                existingAlert.lastSeenTime = new Date().getTime(); // Update last seen time
        ...
        // Check for alerts that have been inactive for more than 30 seconds
        if (this.appState.activeAlerts) {
            ...
            this.appState.activeAlerts.forEach(alert => {
                ...
                if (!stillCritical) {
                    // If the utility isn't in proximity anymore, check how long it's been missing
                    const timeSinceLastSeen = now - alert.lastSeenTime;
                    if (timeSinceLastSeen > 30000) { // 30 seconds in milliseconds
                        alertsToRemove.push(alert);
                    }
                }
            });
            
            // Remove alerts that have been inactive for too long
    ...
    addUserLocationMarker(lat, lng) {
        ...
        // Create a marker with a premium design and clear heading indicator
        const userLocationIcon = L.divIcon({
            className: 'user-location-marker with-heading', // Ensure 'with-heading' class
            html: `
                <div class="location-marker-container">
                    <div class="location-accuracy-circle" style="position: absolute; width: 100%; height: 100%; border-radius: 50%; background-color: rgba(33, 150, 243, 0.15); animation: accuracy-pulse 3s infinite;"></div>
                    <div class="location-pulse" style="position: absolute; width: 100%; height: 100%; border-radius: 50%; background-color: rgba(33, 150, 243, 0.2); animation: location-pulse 2s infinite;"></div>
                    <div class="location-pulse-inner" style="position: absolute; width: 70%; height: 70%; left: 15%; top: 15%; border-radius: 50%; background-color: rgba(33, 150, 243, 0.3); animation: location-pulse 2s infinite 0.3s;"></div>
                    <div class="location-center" style="position: absolute; width: 10px; height: 10px; border-radius: 50%; background-color: #2196F3; top: 50%; left: 50%; transform: translate(-50%, -50%); border: 2px solid white; box-shadow: 0 0 6px rgba(0,0,0,0.3); animation: center-pulse 2s infinite;"></div>
                    <div class="location-heading" style="position: absolute; top: -28px; left: 50%; width: 0; height: 0; border-left: 14px solid transparent; border-right: 14px solid transparent; border-bottom: 28px solid #2196F3; transform-origin: bottom center; transform: translateX(-50%) rotate(0deg); opacity: 1; z-index: 1001; transition: transform 0.2s ease-out; box-shadow: 0 3px 8px rgba(0,0,0,0.3); filter: drop-shadow(0 0 1px rgba(255,255,255,0.8));"></div>
                </div>
            `,
            iconSize: [48, 48], // Slightly larger for better visibility
            iconAnchor: [24, 24]
        });
        ...
        // Apply an initial test heading to verify visibility
        setTimeout(() => {
            this.updateHeading(45);
            console.log('Applied test heading: 45 degrees');
        }, 500);
