...
export class MapController {
            ...
            if (this.userLocationMarker) {
                ...
                if (markerElement) {
                    ...
                    if (headingElement) {
                        ...
                        // Force transform with !important to override any CSS rules
                        headingElement.style.cssText += `; transform: translateX(-50%) rotate(${this.currentHeading}deg) !important;`;
    ...
    addOrientationListener() {
        ...
        // Always attempt to listen for BOTH possible event types. In many mobile
        // browsers `ondeviceorientationabsolute` is defined but no events are
        // ever dispatched, which causes the heading to stay at 0°.  By adding
        // listeners for *both* event types we guarantee we will receive data
        // from whichever API is actually implemented on the client.

        const eventTypes = [];

        if ('ondeviceorientationabsolute' in window) {
            console.log('deviceorientationabsolute API detected – adding listener');
            eventTypes.push('deviceorientationabsolute');
        }

        if ('ondeviceorientation' in window) {
            console.log('deviceorientation API detected – adding listener');
            eventTypes.push('deviceorientation');
        }
        ...
        // Register listeners and monitor whether we actually get events back.
        let eventCount = 0;
        eventTypes.forEach(type => {
            window.addEventListener(type, this.orientationHandler, false);
            // Small utility listener just to count incoming events.
            const counter = () => { eventCount++; };
            window.addEventListener(type, counter);
            // Remove the temporary counter listener after 2 s.
            setTimeout(() => {
                window.removeEventListener(type, counter);
            }, 2000);
        });

        this.isOrientationActive = true;

        // After 2 s see if we actually received anything. If not, warn user so
        // they know why the arrow might not move.
        setTimeout(() => {
            console.log(`Orientation events received in first 2 s: ${eventCount}`);
            if (eventCount === 0 && this.uiController) {
                this.uiController.showToast('Compass data not available – heading will not update', 'warning');
            }
        }, 2100);
    }
    ...
    handleOrientation(e) {
        ...
        // Alpha without correction is generally not useful, 
        // need to apply device & screen orientation corrections
        
        // Android/Chrome approach with absolute orientation
        if (e.alpha !== null && e.alpha !== undefined) {
            let heading;
            let source = '';
            
            // Absolute orientation API (most accurate non-iOS approach)
            if (e.absolute === true) {
                heading = 360 - e.alpha;
                source = 'absolute';
            } 
            // Apply screen orientation correction if available
                ...
                source = 'screen-corrected';
            } 
            // Try to use device orientation for correction
            else if (e.beta !== null && e.gamma !== null) {
                // Use accelerometer to determine if device is in portrait/landscape
                const portrait = Math.abs(e.beta) > 45;
                let orientation = 0;
                
                if (portrait) {
                    // Portrait orientation
                    if (e.beta < 0) orientation = 180;
                } else {
                    // Landscape orientation
                    orientation = e.gamma > 0 ? 90 : 270;
                }
                
                heading = 360 - e.alpha;
                heading = (heading + orientation) % 360;
                source = 'accelerometer-corrected';
            }
            // Last resort - just use alpha with a basic correction
            else {
                heading = 360 - e.alpha;
                source = 'basic-alpha';
            }
                    ...
                    this.watchPositionId = navigator.geolocation.watchPosition(
                        (position) => {
                            ...
                            if (position.coords.heading !== null && position.coords.heading !== undefined) {
                                this.processHeading(position.coords.heading);
                            }
