...
export class MapController {
    constructor(appState, dataStore) {
        ...
        // Heading tracking properties - FIXED: Added missing initializations
        this.lastHeadingSource = null;
        this.lastHeadingTime = 0;
        this.lastHeadingPriority = null;
        this.lastHighPriorityTime = 0;
        ...
        if ('geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    const errorMessage = this.getPositionErrorMessage(error);
                    
                    if (this.uiController) {
                        this.uiController.showToast('Could not get your location: ' + errorMessage, 'error');
                    }
                    
                    // Fallback: try to get cached location or use default
    ...
    /**
     * Handle location fallback when geolocation fails
     */
    handleLocationFallback() {
        // Try to use last known location if available
        if (this.appState.userLocation) {
            this.map.setView(this.appState.userLocation, this.map.getZoom());
            if (this.uiController) {
                this.uiController.showToast('Using last known location', 'info');
            }
        } else {
            // Default to a central location (Washington DC)
            this.map.setView([38.897957, -77.036560], 16);
            if (this.uiController) {
                this.uiController.showToast('Using default location. Please enable location services.', 'warning');
            }
        }
    }
    
    /**
     * Get user-friendly error message for position errors
     */
            ...
            if (this.userLocationMarker) {
                ...
                if (markerElement) {
                    ...
                    if (headingElement) {
                        ...
                        console.warn('❌ Heading element NOT found in marker - creating it');
                        this.createHeadingElement(markerElement, heading);
                    }
    ...
    /**
     * Create heading element if it doesn't exist (fallback)
     * @param {HTMLElement} markerElement The marker element
     * @param {number} heading The current heading
     */
    createHeadingElement(markerElement, heading) {
        try {
            ...
            console.log('✅ Created missing heading element');
        } catch (error) {
            console.error('Error creating heading element:', error);
        }
    }
    ...
    startProximityMonitoring() {
        ...
        // Start a new interval to check proximity - FIXED: Added error handling
        this.proximityInterval = setInterval(() => {
            try {
                ...
                // Don't stop monitoring, just log the error
            }
    ...
    stopHighAccuracyLocationTracking() {
        ...
        if (this.orientationHandler && this.isOrientationActive) {
            try {
                ...
                this.isOrientationActive = false;
                this.orientationHandler = null;
                console.log('Orientation listeners cleaned up');
            } catch (error) {
                console.warn('Error cleaning up orientation listeners:', error);
            }
        }
    ...
    stopWatchingPosition() {
        ...
        // Remove orientation listener if added - FIXED: Improved cleanup
        if (this.orientationHandler && this.isOrientationActive) {
            try {
                window.removeEventListener('deviceorientation', this.orientationHandler);
                window.removeEventListener('deviceorientationabsolute', this.orientationHandler);
        ...
        if (this.signalStrengthIndicator) {
            try {
                ...
                console.warn('Error removing signal strength indicator:', error);
            }
        }
