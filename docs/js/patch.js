        ...
        if (!window.mapController.updateHeading || typeof window.mapController.updateHeading !== 'function') {
            window.mapController.updateHeading = function(heading) {
                ...
                if (this.userLocationMarker) {
                    ...
                    if (markerElement) {
                        // Add the with-heading class if not present
                        ...
                        if (headingElement) {
                            // Apply strong styling to arrow to help ensure it's visible
                            ...
                            headingElement.style.borderLeft = '8px solid transparent';
                            headingElement.style.borderRight = '8px solid transparent';
                            headingElement.style.borderBottom = '16px solid #2196F3';
                            headingElement.style.transformOrigin = 'bottom center';
                            
                            // Update the transform - use CSS transform for better performance
                            ...
                            console.log('[patch] Applied heading rotation:', heading, 'degrees');
                        } else {
                            console.error('[patch] Heading element not found in marker - creating it');
                            
                            // Try to create the arrow if it doesn't exist
                            const newHeading = document.createElement('div');
                            newHeading.className = 'location-heading';
                            newHeading.style.position = 'absolute';
                            newHeading.style.width = '0';
                            newHeading.style.height = '0';
                            newHeading.style.left = '50%';
                            newHeading.style.top = '-10px';
                            newHeading.style.borderLeft = '8px solid transparent';
                            newHeading.style.borderRight = '8px solid transparent';
                            newHeading.style.borderBottom = '16px solid #2196F3';
                            newHeading.style.transformOrigin = 'bottom center';
                            newHeading.style.transform = `translateX(-50%) rotate(${heading}deg)`;
                            newHeading.style.opacity = '1';
                            newHeading.style.zIndex = '1000';
                            newHeading.style.display = 'block';
                            
                            // Add to marker
                            markerElement.appendChild(newHeading);
                        }
                    }
                }
            };
